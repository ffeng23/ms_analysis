...
df <- diann_load(here(data.dir,
	"CD4+T_activation_05102024_Report.tsv"))
...
df.long<- df %>% #dplyr::select(!starts_with("HeLa")) %>%
	pivot_longer(cols=contains("CD4+T"),
	names_to="Run", values_to = "PG.MaxLFQ"
	)
#cleanup the record and keep for now only PG.MaxLFQ
# Run, 
df.clean <- df.long %>% 
	dplyr::select(c("Run","PG.ProteinGroups",
		#"Protein.Ids", #"Protein.Names",
		"PG.Genes",	
		#"Precursor.Id",
	 "PG.MaxLFQ")) %>%
	#mutate(Run2=sub(x=Run, pattern="\\[[0-9]+\\].raw\\.PG\\.Quantity$","")) %>%
	mutate(Run=sub(x=Run, pattern="^\\[[0-9]+\\] CD4\\+T_","")) %>%
	dplyr::filter(!grepl(x=Run, pattern="(pH6|IL2|resting|dynabead)"))%>%
	...
	mutate(Run2=sub(x=Run,pattern="_[0-9]{8}\\.raw\\.PG\\.Quantity$","")) %>%
	mutate(Repeat=sub(x=Run2, pattern="^[0-9a-z_]+\\-","")) %>%
	
	#mutate(Treatment=sub(x=Treatment, pattern="\\-[1-3]{1}$","")) %>%
	mutate(Treatment="Activated") %>%
	mutate(Run2=sub(x=Run2, pattern="\\-[1-3]{1}$","")) %>% 
	mutate(Run2=sub(x=Run2, pattern="naive_24h","actived_0min")) %>%
	mutate(Time=sub(x=Run2, pattern="actived_","")) #%>%
...
trend.vista.wide<-items.means.wide %>% pivot_wider(
		id_cols=c(Protein.Group,Genes), 
		values_from=mean_Activated, names_from=Time
	) %>% #mutate("0"=0) %>% 
	relocate("5min",.before="10min")%>%

	relocate("0min",.before="5min")

trend.vista.wide<- trend.vista.wide %>%
	mutate(across(contains("min"), ~.-`0min`))
...
trend.vista.wide$`0min`<-#trend.vista.wide$"0"+
	rnorm(length(trend.vista.wide$`0min`),0,0.000001)

#save 
saveRDS(trend.vista.wide, 
	file=here(output.dir,"Trend.time.RDS"))
	
pdf(file=here(output.dir,"Timeeffect_raw_hier.pdf"),
	width=7, height=7)
...
dev.off()
pdf(file=here(output.dir,"Timeeffect_kmean.pdf"),
	width=7, height=4)
...
trend.vista.long<- trend.vista.wide %>%
	pivot_longer(cols=c("0min","5min","10min","15min","20min"),
		names_to="time_point",
...
trend.vista.long$time_point<-factor(trend.vista.long$time_point,
		levels=c("0min","5min","10min","15min","20min"))
...
pdf(file=here(output.dir,"Timeeffect_kmean_lines_k10.pdf"),
	width=10, height=6)
ggplot(		
	)+
