...
source("functions.R")
#read the data
df <- diann_load(here(data.dir,"Kasumi3_SNS-101_Timecourse_05272023_report_PG.MaxLFQ_pivoted.csv"))
df<-removeReplicates(df) 

#pivot the table back to make the flow as before.
df.long<- df %>% pivot_longer(cols=starts_with("Kasumi3"),
...
saveRDS(df.proteome.stat, file=here(output.dir,"stat_proteome_fill_pivotData.Rds"))
#df.proteome.stat<-readRDS(file=here(output.dir,"stat_proteome_fill.Rds"))
write_csv(df.proteome.stat, 
	file=here(output.dir,"stat_proteome_fill_pivotData.csv"))
